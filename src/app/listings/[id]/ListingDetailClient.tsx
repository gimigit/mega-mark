...
export default function ListingDetailClient({ listingId }: { listingId: string }) {
  ...
  useEffect(() => {
    const fetchListing = async () => {
      const { data, error } = await supabase
        .from('listings')
        .select(
          '*, profiles(full_name, avatar_url, rating_avg, verified, location_country), categories(name, icon)'
        )
        .eq('id', listingId)
        .single()
  ...
  const handleContactClick = () => {
    ...
    if (!listing) {
      setToast({ message: 'Anunțul nu a fost încărcat.', type: 'error' })
      return
    }
    if (user.id === listing.seller_id) {
      setToast({ message: 'Nu poți trimite mesaje către tine însuți.', type: 'error' })
      return
    }
  ...
  const handleSendMessage = async () => {
    if (!messageContent.trim()) {
      setToast({ message: 'Te rugăm să scrii un mesaj.', type: 'error' })
      return
    }
    ...
    const { error } = await supabase.from('messages').insert({
      ...
      if (error) {
        setToast({ message: 'Eroare la trimiterea mesajului. Încearcă din nou.', type: 'error' })
      } else {
        setToast({ message: 'Mesaj trimis!', type: 'success' })
  ...
  const getShareTitle = () => {
    return listing ? `${listing.title} | AgroMark EU` : 'AgroMark EU'
  }
  ...
  const getShareDescription = () => {
    ...
    return `€${listing.price.toLocaleString()} — ${parts.join(' · ')}`
  }
  ...
  return (
    <div className="min-h-screen bg-gray-50">
              ...
              {images && images.length > 0 ? (
                <>
                  <div className="relative bg-gray-100 aspect-[16/9] flex items-center justify-center">
                    ...
                    />
                    <span className={`absolute top-4 left-4 px-3 py-1.5 rounded-full text-xs font-bold text-white ${statusColors[listing.status] || 'bg-gray-500'}`}>
              ...
              <div className="mt-4 border-t border-gray-100 pt-4">
                ...
                <div className="flex items-center gap-2">
                  <button
                    ...
                    className="flex-1 py-2 border border-gray-200 rounded-lg text-sm font-semibold text-gray-600 hover:bg-blue-50 hover:border-blue-300 hover:text-blue-700 transition-colors flex items-center justify-center gap-1"
                  ...
                  <button
                    ...
                    className="flex-1 py-2 border border-gray-200 rounded-lg text-sm font-semibold text-gray-600 hover:bg-sky-50 hover:border-sky-300 hover:text-sky-700 transition-colors flex items-center justify-center gap-1"
                    title="Distribuie pe Twitter/X"
                  >
                    <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 24 24"><path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-4.714-6.231-5.401 6.231H2.742l7.727-8.435L1.254 2.25H8.08l4.253 5.622 5.911-5.622Zm-1.161 17.52h1.833L7.084 4.126H5.117z"/></svg>
                    X
                  </button>
                  <button
                    onClick={shareOnWhatsApp}
                    className="flex-1 py-2 border border-gray-200 rounded-lg text-sm font-semibold text-gray-600 hover:bg-green-50 hover:border-green-300 hover:text-green-700 transition-colors flex items-center justify-center gap-1"
      ...
      {toast && (
        <div className={`fixed bottom-6 right-6 z-50 px-6 py-4 rounded-xl shadow-lg text-white font-medium animate-in slide-in-from-bottom-5 duration-300 ${toast.type === 'success' ? 'bg-green-600' : 'bg-red-600'}`}>
          {toast.message}
        </div>
      )}
    </div>
